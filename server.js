...
app.post('/api/history', (req, res) => {
  try {
    ...
    const itemHistoryDir = path.join(historyDir, item.id);
    if (!fs.existsSync(itemHistoryDir)) {
      fs.mkdirSync(itemHistoryDir, { recursive: true });
    }

    // Process results array and extract images
      ...
      item.results.forEach((result, index) => {
        if (result.imageUrl && result.imageUrl.startsWith('data:image')) {
          ...
          if (matches && matches.length === 3) {
            ...
            const imageName = `image_${index}.${ext}`;
            const imagePath = path.join(itemHistoryDir, imageName);
            fs.writeFileSync(imagePath, buffer);
            // Replace base64 URL with API endpoint
            result.imageUrl = `/api/history/${item.id}/image/${imageName}`;
          }
        }
      });
    }
...
app.get('/api/history/:id/image/:imageName', (req, res) => {
  try {
    const { id, imageName } = req.params;
    // Basic sanitization
    if (id.includes('..') || id.includes('/') || imageName.includes('..') || imageName.includes('/')) {
      return res.status(400).send('Invalid path');
    }
    const imagePath = path.join(historyDir, id, imageName);
    ...
    console.error('Error serving history image:', error);
...
app.delete('/api/history/:id', (req, res) => {
  try {
    ...
    const itemHistoryDir = path.join(historyDir, id);
    ...
    if (fs.existsSync(itemHistoryDir)) {
      fs.rmSync(itemHistoryDir, { recursive: true, force: true });
      deleted = true;
    }
