...
export const ImageGenerator: React.FC = () => {
    ...
    // History functionality is now handled via Home page and Image Details
    ...
    const handleGenerate = async () => {
        ...
        try {
            for (const company of companies) {
                ...
                const companyPrompts = ratios.map(async (ratio): Promise<GeneratedResult | null> => {
                    ...
                    const fullPrompt = activeTab === 'new'
                        ? `A promotional marketing image. ${backgroundInstruction} ${themeInstruction} ${brandingInstruction} Text overlay: "${tagline}". Commercial photography style, high resolution.`
                        : `A promotional marketing image using the provided product. ${backgroundInstruction} ${themeInstruction} ${brandingInstruction} The product must be centrally positioned and fully visible. Text overlay: "${tagline}". Commercial photography style.`;

                    const imgData = await generateImageWithAssets(fullPrompt, assets, ratio);

                    if (imgData) {
                        // Handle potential object return from new API
                        const imageUrlString = typeof imgData === 'string' ? imgData : imgData.imageBase64;
                        const usedRatio = typeof imgData === 'string' ? ratio : imgData.usedRatio;

                        return {
                            companyId: company.id,
                            companyName: company.name,
                            ratio: usedRatio, // Use actual ratio
                            imageUrl: `data:image/jpeg;base64,${imageUrlString}`,
                            warning: usedRatio !== ratio ? `Ratio adjusted: ${ratio} → ${usedRatio}` : undefined
                        };
                    }
                    return null;
                });
    ...
    return (
        <div className="max-w-7xl mx-auto p-6">
            <h2 className="text-3xl font-bold text-gray-800 mb-6">Image Generator</h2>
            ...
            <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg mb-8 w-fit">
                ...
                >
                    <ImageIcon size={18} /> Include Product in Image
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 h-fit lg:col-span-1">
                    ...
                    <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <button
                                ...
                                className={`flex items-center justify-center gap-2 py-3 rounded-lg border transition-all ${themeMode === 'light'
                            ...
                            <button
                                ...
                                className={`flex items-center justify-center gap-2 py-3 rounded-lg border transition-all ${themeMode === 'dark'
                    ...
                    <div className="mb-6">
                        <h3 className="text-sm font-bold text-gray-700 mb-2 uppercase">
                            {activeTab === 'include_product' ? '3. Image Tagline' : '2. Image Tagline'}
                        </h3>
                        <textarea
                            value={tagline}
